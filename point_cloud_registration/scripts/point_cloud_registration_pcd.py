...
if __name__ == "__main__":
    ...
    for point_cloud_source_filename, point_cloud_target_filename, gps_data_filename, imu_data_filename in zip(
            sorted(os.listdir(folder_path_point_cloud_source)), sorted(os.listdir(folder_path_point_cloud_target)),
            sorted(os.listdir(folder_path_gps_data)), sorted(os.listdir(folder_path_imu_data))):
        print("processing point cloud with index: ", str(idx_file))
        if idx_file > 0 and idx_file <550 :
            ...
            gps_json = json.load(open(os.path.join(folder_path_gps_data, gps_data_filename)))
            imu_json = json.load(open(os.path.join(folder_path_imu_data, imu_data_filename)))
            ...
            if transformation_matrix is not None:
                ...
                # transformation_matrix_rtk_to_robosense = np.array([[0.98064662, 0.17906124, -0.07917877, 0.02639554],
                #                                                    [-0.17893175, 0.98382226, 0.00878538, 0.94303686],
                #                                                    [0.07947096, 0.00555224, 0.99682172, -0.14263901],
                #                                                    [0.00000000, 0.00000000, 0.00000000, 1.00000000]],
                #                                                   dtype=float)
                source_transformed = np.matmul(transformation_matrix_rtk_to_robosense, point_cloud_source_homogeneous.T).T
                source_transformed = np.matmul(transformation_matrix, source_transformed.T).T
